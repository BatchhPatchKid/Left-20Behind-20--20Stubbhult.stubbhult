...
if (isNull (currentTask _caller)) then {
    ...
    private _wpPos = [_pos,200,400] call _fn_findSafePosInBounds;
    if ((_wpPos distance2D [0,0,0]) <= 1 || {!([_wpPos] call _fn_inBounds)}) then { _wpPos = _pos; };

    _waypoint1 = _sniperGrp addWaypoint [_wpPos,0];
