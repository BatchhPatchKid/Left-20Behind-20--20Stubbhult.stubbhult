...
FN_PurchaseMag_Crate={
	...
	private _slot=_args select 0;
	private _cost=_args select 1;
	[_target,_caller,_slot,_cost] remoteExec ["LB_fnc_tryPurchaseMagCrateServer",2];
};
...
// Keeping it as a harmless stub so any legacy calls do not break.
FN_addMoney={
...
FN_SellAction_Crate={
	...
	{
		_contents pushBack [(_x select 0),1];
	} forEach (weaponsItemsCargo _container);

	private _bp=getBackpackCargo _container;
	{
		_contents pushBack [_x,(_bp select 1) select _forEachIndex];
	} forEach (_bp select 0);

	private _mg=getMagazineCargo _container;
	{
		_contents pushBack [_x,(_mg select 1) select _forEachIndex];
	} forEach (_mg select 0);

	private _it=getItemCargo _container;
	{
		_contents pushBack [_x,(_it select 1) select _forEachIndex];
	} forEach (_it select 0);

	// Any rvg_money already in the crate is treated as cash value (1:1) and paid to the seller.
	private _existingMoney=([_mg,_currency] call _fn_getCargoCount)+([_it,_currency] call _fn_getCargoCount);
	...
	private _finalMoney=_existingMoney+(_payout*3);

	if (_finalMoney>0) then {
		[_caller,_finalMoney] remoteExec ["LB_fnc_addMoneyServer",2];
	if (_payout>0) then {
		hintSilent format ["%1$ has been added to your account. Pleasure doing business with you %2",_finalMoney,name _caller];
	} else {
		hintSilent format ["No sellable items found. Existing money (%1$) was preserved for you %2",_finalMoney,name _caller];
	};
	} else {
...
FN_services_Crate = {
	...
	_containerAction = _container addAction ["Purchase PARDON (Remove Renegade Status): $250", FN_PurchaseService_Crate, ["clear_renegade",250],1.5,true,false,"","true",3];
	_containerAction = _container addAction ["Purchase DIPLOMATIC IMMUNITY (Permanent Friendly with Trader's Faction): $2000", FN_PurchaseService_Crate, ["perm_friendly_trader_faction",2000],1.5,true,false,"","[_target, _this] call (missionNamespace getVariable 'FN_canPurchasePermanentAllianceAtCrate')",3];
	_containerAction = _container addAction ["Disband Permanent Alliance (Trader Faction) (+$2000)", FN_disbandPermanentAllianceAtCrate, [],1.5,true,false,"","[_target, _this] call (missionNamespace getVariable 'FN_canDisbandPermanentAllianceAtCrate')",3];
