...
// Tuning knobs (1.0 = current behavior)
// example use: 
// 1. missionNamespace setVariable ["LB_tempCoolMult",1.0];
// 2. missionNamespace setVariable ["LB_tempHeatMult",1.0];
if (isNil {missionNamespace getVariable "LB_tempCoolMult"}) then {missionNamespace setVariable ["LB_tempCoolMult",1.0,true];};
if (isNil {missionNamespace getVariable "LB_tempHeatMult"}) then {missionNamespace setVariable ["LB_tempHeatMult",1.0,true];};
...
getCalculatedBodyTempAndHealth = {
    ...
    // 1.0 = current behavior, < 1.0 slower, > 1.0 faster
    private _coolMult = 0.8;
    private _heatMult = 0.8;
    ...
    _inBuilding = (count _nbuilding > 0);
    ...
    if (_isFire) then {
        if (bodyTemp < 98.6) then {
            bodyTemp = bodyTemp + (0.25 * _heatMult);
            if (_inBuilding) then { _fireCancel = .01; };
        } else {
            bodyTemp = bodyTemp + (0.005 * _heatMult);
        };
    };

    if (_time < 4 || _time > 20) then {
    ...
    if (surfaceIsWater getPos player && isNull objectParent player && !(_checkStats)) then {
        bodyTemp = bodyTemp - (0.3 * _coolMult);
    };
    ...
    if !(_checkStats) then {
        bodyTemp = bodyTemp - (_bodyTempCalc * _coolMult);
        ...
        if (bodyTemp < 91) then {
            ...
            if ((round (random 20)) == 1) then { [player,true,15] call ace_medical_fnc_setUnconscious; };
        };
