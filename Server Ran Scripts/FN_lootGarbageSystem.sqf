...
while {true} do {
	{
		...
		private _cands = nearestObjects [_player, [], _scanRadius];
		{
            ...
            private _middlePos = _obj modelToWorld [
            ...
            if (_class in _lootStems) then { 
                private _existing = nearestObjects [_middlePos, ["Land_HelipadEmpty_F"], 10];
                ...
                if (_existing isEqualTo []) then {
                    private _hp = createVehicle ["Land_HelipadEmpty_F", _middlePos, [], 0, "CAN_COLLIDE"];
                    _hp setVariable ["interactionPoint", 0, true];

                    private _chem = createVehicle ["Chemlight_green", _middlePos, [], 0, "CAN_COLLIDE"];
                };
            };

            if (_class in _waterStems) then {
                private _existing = nearestObjects [_middlePos, ["Land_HelipadEmpty_F"], 1];
                ...
                if (_existing isEqualTo []) then {
                    ...
                    private _chem = createVehicle ["Chemlight_blue", _middlePos, [], 0, "CAN_COLLIDE"];
                };
            };
		} forEach _cands;
