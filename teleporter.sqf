...
player setPos (getPos teleporterArea);
...
if (_typeOfTeleporter) then {
	...
	_factionArray = ["BB",.25, "Bandit",.25, "DT",.25, "NH",.25, "PF",.15, "TRB",.10, "ALF",.10, "WO",.10, "RC",.10, "SU",.10];
	...
	if (!(("ItemMap" in assignedItems _player) || ("ItemMap" in items player))) then {
