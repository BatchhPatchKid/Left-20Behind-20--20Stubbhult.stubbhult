...
if (isNil "_primaryOverride" || { !(_primaryOverride isEqualType true) }) then { _primaryOverride = false; };
if (isNil "_sfOverride" || { !(_sfOverride isEqualType true) }) then { _sfOverride = false; };

private _secondary = false;

private _fn_isSpecialUnit = {
	params ["_chanceDiv","_melee","_sfOverride"];
	_sfOverride || ((!_melee) && { (random _chanceDiv) < 1 })
};
...
_gpsArraySelection = ["DSA_Detector",0.15,"ItemGPS",1];
...
switch _faction do {
	case "Bandit": {
		...
		if ([50,_melee,_sfOverride] call _fn_isSpecialUnit) then {
			_firstAid = true;
			_water = true;
			_food = true;
			_money = true;
	...
	case "PF": {
		...
		if ([30,_melee,_sfOverride] call _fn_isSpecialUnit) then {
			_firstAid = true;
			_water = true;
			_food = true;
			_money = true;
	...
	case "RC": {
		...
		if ([40,_melee,_sfOverride] call _fn_isSpecialUnit) then {
			_firstAid = true;
			_water = true;
			_food = true;
			_money = true;
	...
	case "US": {
		...
		if ([10,_melee,_sfOverride] call _fn_isSpecialUnit) then {
			_firstAid = true;
			_water = true;
			_food = true;
			_money = true;
	...
	case "RU": {
		...
		if ([15,_melee,_sfOverride] call _fn_isSpecialUnit) then {
			_firstAid = true;
			_water = true;
			_food = true;
			_money = true;
	...
	case "ROA": {
		...
		if ([20,_melee,_sfOverride] call _fn_isSpecialUnit) then {
			_firstAid = true;
			_water = true;
			_food = true;
			_money = true;
...
if (_firstAid) then { _unit addItemToBackpack (selectRandomWeighted _firstAidSelection); };
if (_water) then { _unit addItemToBackpack (selectRandomWeighted _waterSelection); };
if (_food) then { _unit addItemToBackpack (selectRandomWeighted _foodSelection); };
if (_grenades) then { _unit addItemToBackpack (selectRandomWeighted _grenadeItems); };

if (_money) then {
	for "_j" from 1 to (round (random 5)) do { _unit addItemToVest "rvg_money"; };
};

if (_nvg) then { _unit linkItem (selectRandomWeighted _nvgSelection); };
...
if (!(_melee)) then {
	if (_primary || _primaryOverride || _sfOverride) then {
		...
		if (_optics) then {
			...
			if (_rifle in _longRangeWeapons) then {
		...
		if (_muzzle) then { _unit addPrimaryWeaponItem (selectRandomWeighted _muzzleArraySelection); };
	} else {
...
if (_rifle in _glWeapons) then {
	...
	if (_rifle in _glRussian) then {
		...
		for "_i" from 1 to _numRounds do { _unit addItemToBackpack (selectRandomWeighted _glRounds); };
	} else {
...
if (_muzzle) then { _unit addHandgunItem (selectRandomWeighted _muzzleArraySelection); };

for "_i" from 1 to _rndShitAmount do { _unit addItemToBackpack (selectRandomWeighted _rndShitArraySelection); };

if (_map) then { _unit linkItem "ItemMap"; };
if (_compass) then { _unit linkItem "ItemCompass"; };
if (_watch) then { _unit linkItem "ItemWatch"; };
if (_radio) then { _unit linkItem (selectRandomWeighted _radioArraySelection); };
if (_gps) then { _unit linkItem (selectRandomWeighted _gpsArraySelection); };